...
package com.cliqconsulting.cclib.google.games;
...
public class GameHelper implements GoogleApiClient.ConnectionCallbacks,
	...
	public void onStart(Activity act) {
		...
		if (mConnectOnStart) {
			if (mGoogleApiClient.isConnected()) {
				CCLog.logWarn("[GAMEHELPER] GameHelper: client was already connected on onStart()");
			} else {
	...
	public String getInvitationId() {
		if (!mGoogleApiClient.isConnected()) {
			CCLog.logWarn("[GAMEHELPER] Warning: getInvitationId() should only be called when signed in, " +
	...
	public Invitation getInvitation() {
		if (!mGoogleApiClient.isConnected()) {
			CCLog.logWarn("[GAMEHELPER] Warning: getInvitation() should only be called when signed in, " +
					"that is, after getting onSignInSuceeded()");
		}
		return mInvitation;
	}
	...
	public TurnBasedMatch getTurnBasedMatch() {
		if (!mGoogleApiClient.isConnected()) {
			CCLog.logWarn("[GAMEHELPER] Warning: getTurnBasedMatch() should only be called when signed in, " +
					"that is, after getting onSignInSuceeded()");
		}
	...
	public void enableDebugLog(boolean enabled, String tag) {
		CCLog.logWarn("[GAMEHELPER] GameHelper.enableDebugLog(boolean,String) is deprecated. " +
	...
	public void reconnectClient() {
		if (!mGoogleApiClient.isConnected()) {
			CCLog.logWarn("[GAMEHELPER] reconnectClient() called when client is not connected.");
	...
	public void disconnect() {
		if (mGoogleApiClient.isConnected()) {
			...
			CCLog.logWarn("[GAMEHELPER] disconnect() called when client was already disconnected.");
		}
	}
	...
	public static void showFailureDialog(Activity activity, int actResp, int errorCode) {
		if (activity == null) {
			CCLog.logError("[GAMEHELPER] *** No Activity. Can't show failure dialog!");
			return;
		}
		...
		switch (actResp) {
			...
			default:
				...
				if (errorDialog == null) {
					// get fallback dialog
					CCLog.logError("[GAMEHELPER] No standard error dialog available. Making fallback dialog.");
	...
	void debugLog(String message) {
		if (mDebugLog) {
			CCLog.logDebug("[GAMEHELPER] GameHelper: " + message);
		}
	}

	void logWarn(String message) {
		CCLog.logWarn("[GAMEHELPER] GameHelper WARNING: " + message);
	}

	void logError(String message) {
		CCLog.logError("[GAMEHELPER] GameHelper ERROR: " + message);
	}
